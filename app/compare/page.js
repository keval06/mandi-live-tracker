...
import { normalizeRecords, groupByCommodity, avgModalPrice, formatRupee } from '@/lib/api/utils.js'
...
function buildTrend(records, commodity) {
  ...
  return Object.entries(byDate)
    .sort(([a],[b]) => { const ms=s=>{const[d,m,y]=s.split('/');return new Date(`${y}-${m}-${d}`).getTime()};return ms(a)-ms(b) })
    .map(([date,prices]) => ({ date, avgPrice: Math.round(prices.reduce((s,p)=>s+p,0)/prices.length) }))
}

function buildPerMandi(records, commodity) {
  return records.filter(r=>r.commodity===commodity&&r.modalPrice>0)
...
export default async function ComparePage({ searchParams }) {
  ...
  if (!state) {
    return (
      <div className="max-w-5xl mx-auto px-6 py-6">
        <h1 className="text-lg font-medium text-gray-900 mb-1">Compare commodities</h1>
        <p className="text-xs text-gray-400 mb-5">Select a state first to load commodities</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {stateTotals.map(s => (
            <a key={s.state} href={`/compare?state=${encodeURIComponent(s.state)}`}
  ...
  const { records: raw } = await getPricesByState(state)
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-6">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[{c:sel1,s:stats(sel1)},{c:sel2,s:stats(sel2)}].map(({c,s}) => (
          <div key={c} className="bg-white border border-gray-100 rounded-lg p-4">
            <p className="text-sm font-medium text-gray-900 mb-3">{c || '—'}</p>
            {!s ? <p className="text-xs text-gray-300">No data</p> : (
      ...
      <div className="grid grid-cols-2 gap-3">
        <TrendChart commodity={sel1} data={buildTrend(records,sel1)} perMandi={buildPerMandi(records,sel1)} />
        <TrendChart commodity={sel2} data={buildTrend(records,sel2)} perMandi={buildPerMandi(records,sel2)} />
      </div>
    </div>
  )
}