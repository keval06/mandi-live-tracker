...
//* Object -> Token bucket — allows short bursts, throttles sustained load
const bucket = { 
  tokens: 5, 
  last: Date.now(),
};

const REFILL_RATE = 400; // 1 token per 400ms = 2.5/sec

async function acquireToken() {
  const now = Date.now();
  const refill = Math.floor((now - bucket.last) / REFILL_RATE);
...
async function fetchWithRetry(url, retries = 4) {
  let backoff = 10000; // miliseconds, start at 10s — short waits waste retries when IP is blocked
  
  for (let i = 0; i < retries; i++) {
    const res = await fetch(url, { next: { revalidate: 21600 } }); // 6h — data.gov.in updates once daily

    if (res.status === 429 && i < retries - 1) {
      console.warn(`[API] Rate limited. Retrying in ${backoff / 1000}s...`, "[lib/api/mandi.js]");
      await sleep(backoff);
      backoff *= 2;
      continue;
    }
...
function buildUrl(filters = {}, offset = 0) {
  const p = new URLSearchParams({
    ...
    _date: todayStr(), // cache-busting: different key each day, yesterday's cache ignored
  });
  for (const [k, v] of Object.entries(filters)) {
    // BUG FIX: data.gov.in API is literally bugged and requires DOUBLE-encoded spaces (%2520)
    // for Mandi names (e.g. Surat%2520APMC). 
    // encodeURIComponent(v) makes it "Surat%20APMC", and p.append() safely encodes it to "Surat%2520APMC".
    if (v) p.append(`filters[${k}]`, encodeURIComponent(v));
  }
...
async function getTotal(filters = {}) {
  ...
  console.log(
    `[API] Fetching Total: ${url.replace(/api-key=[^&]+/, "api-key=***")}`, "[lib/api/mandi.js]"
  );

  const res = await fetchWithRetry(url);

  if (!res.ok) {
    console.error(`[API] Total Fetch Error: ${res.status} ${res.statusText}`, "[lib/api/mandi.js]");
  ...
  try {
    ...
    console.error(`[API] Total Parsing Error: ${e.message}`, "[lib/api/mandi.js]");
    return { total: 0, first: [] };
  }
}
...
async function fetchPage(filters, offset) {
  ...
  if (!res.ok) {
    if (res.status === 429) {
      console.warn(
        `[API] Rate limit hit at offset ${offset} after retries. Skipping.`, "[lib/api/mandi.js]"
      );
    } else {
      console.error(
        `[API] Error ${res.status} at offset ${offset}: ${res.statusText}`, "[lib/api/mandi.js]"
      );
    }
    return [];
  }
  ...
  try {
    ...
    console.error(`[API] Failed to parse JSON at offset ${offset}`, "[lib/api/mandi.js]");
    return [];
  }
}
...
async function fetchAll(filters = {}) {
  ...
  console.log(
    `[Fetch] Total: ${total}. Fetching ${offsets.length} more pages sequentially...`, "[lib/api/mandi.js]"
  );
  ...
  console.log(`[Fetch] Done. Fetched: ${allRecords.length} / ${total}`, "[lib/api/mandi.js]");
...
export function filterByRange(records, range) {
  if (!records || !records.length || !range || range === "All") return records;

  // We find the MAXIMUM date in the dataset.
  // We do NOT use `Date.now()` because the government API may be delayed by a day or more!
  // If we used Date.now(), "Today" would return 0 records when the API hasn't uploaded today's data yet.
  let maxTime = 0;
...
export async function getPricesByMandi(state, mandi, { commodity } = {}) {
  ...
  // Since we now perfectly encode URL spaces to '%20', the API successfully maps the specific market.
  // This bypasses fetching 90 pages for the state, reducing it to ~3 pages per Mandi payload!
  return fetchAll({ state, market: mandi, commodity });
}
